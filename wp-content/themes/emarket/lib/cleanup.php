...
function emarket_body_class($classes) {
	...
	$wishlist_icon = get_option( 'yith_wcwl_add_to_wishlist_icon' );
	if( $emarket_direction == '1' ){
		$classes[] = 'rtl';
	}
	
	if( $wishlist_icon && $wishlist_icon != 'none' ){
		$classes[] = 'wishlist-icon';
	}
	...
	if( $emarket_blog_style == 'home-style57' && !is_front_page() ){
		$classes[] = 'blog-style57';
	}
	if( ( $emarket_blog_style == 'blog-style-service' && is_category() ) || ( $emarket_blog_style == 'blog-style-service' && is_single() )){
		$classes[] = 'blog-style-service';
	}
